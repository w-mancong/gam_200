	...
	namespace ECS
	{
		...
		***********************************************************************************/
		...
		void ColliderSystem::UpdateWorldAxis(Collider2D& collider, Transform const& parentTransform) {
			...
			collider.worldRight(rotationTransform * worldXAxis);
			collider.worldUp(rotationTransform * worldYAxis);
		}
		...
		bool ColliderSystem::CheckCollision_OOBB_To_OOBB(Collider2D const& collider_one, Collider2D const& collider_two, Transform const& parent_transform_one, Transform const& parent_transform_two) {
			...
			if (!CheckIfOverlapAxis(collider_one, collider_two, collider_one.worldRight(), parent_transform_one, parent_transform_two)	||
				!CheckIfOverlapAxis(collider_one, collider_two, collider_one.worldUp(), parent_transform_one, parent_transform_two)		||
				!CheckIfOverlapAxis(collider_one, collider_two, collider_two.worldRight(), parent_transform_one, parent_transform_two)	||
				!CheckIfOverlapAxis(collider_one, collider_two, collider_two.worldUp(), parent_transform_one, parent_transform_two))
			{
		...
		Vector2 ColliderSystem::getMinMax_OOBB_On_Axis(Collider2D box, Vector2 axis, Transform const& parentTransform)
		{
			...
			Vector2 half_right = box.worldRight() * box.scale[0] * 0.5f;
			Vector2 half_up = box.worldUp() * box.scale[1] * 0.5f;
