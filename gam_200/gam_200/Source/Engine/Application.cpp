...
namespace ManCong
{
	...
	{		
		...
		void Application::Init(void)
		{
			...
			m1 = MeshBuilder::Instance()->MakeCircle();
			m1.shader->use();
			m1.shader->Set("view", cam.ViewMatrix());
			m1.shader->Set("proj", cam.ProjectionMatrix());
			m2 = MeshBuilder::Instance()->MakeRectangle();

			img1 = MeshBuilder::Instance()->MakeSprite("Assets/Images/container.jpg");
			img1.shader->use();
			img1.shader->Set("view", cam.ViewMatrix());
			img1.shader->Set("proj", cam.ProjectionMatrix());
			img2 = MeshBuilder::Instance()->MakeSprite("Assets/Images/awesomeface.png");
		}
		...
		void Application::Update(void)
		{
			...
			while (!glfwWindowShouldClose(OpenGLWindow::GetWindow()))
			{
				...
				m1.shader->Set("color", color.r, 0.0f, color.b, color.a);
